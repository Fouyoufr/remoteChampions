...
if (isset($_GET['p'])) $partieId=strtoupper(htmlspecialchars($_GET['p'])); elseif (isset($_POST['p'])) $partieId=strtoupper(htmlspecialchars($_POST['p']));
if (isset($_GET['j'])) $joueurId=htmlspecialchars($_GET['j']); elseif (isset($_POST['j'])) $joueurId=htmlspecialchars($_POST['j']);
if (isset($partieId) and mysqli_num_rows(sql_get("SELECT `pUri` FROM `parties` WHERE `pUri`='$partieId'"))!=1 and !file_exists("ajax/$partieId.xml") ) {
  $badPartie=$partieId;
  unset($partieId);}
if (isset($joueurId)) {
...
<div id='ajaxSave'";
    if (isset($bodyClass)) echo " class='$bodyClass'";
    echo "></div>
<input id='partie' type='hidden' value='".strtoupper($partieId)."'/>";}
if (isset($joueurId)) echo "<input id='jId' type='hidden' value='$joueurId'/>";
echo "
<script language='JavaScript'>
var lang={'hero':'".$str['heros']."','alter':'".$str['alter']."','selectDeck':'".$str['SelectDeck']."','heroSchemes':'".$str['heroSchemes']."'}; 
</script>";}
?>