...
<div id="mechantDisp">
  ...
  <div>
    <input id='vieMechantMoins' type='button' value='<' onclick='document.getElementById("mechantLife").innerText-=1;ajaxPost("vieMechant",document.getElementById("mechantLife").innerText,"ajaxMainSet");'>
    <div id="mechantLife"></div>
    <input class='vieBtn' type='button' value='>' onclick='document.getElementById("mechantLife").innerText=parseInt(document.getElementById("mechantLife").innerText)+1;ajaxPost("vieMechant",document.getElementById("mechantLife").innerText,"ajaxMainSet");'>
  </div>
<?php
echo '<input id="mechantDesoriente" type="button" value="'.$str['villainConfused'].'"  onclick=\'ajaxPost("switch","mechantDesoriente","ajaxMainSet");\'>
  <input id="mechantSonne" type="button" value="'.$str['villainStunned'].'" onclick=\'ajaxPost("switch","mechantSonne","ajaxMainSet");\'>
  <input id="mechantTenace" type="button" value="'.$str['villainTough'].'" onclick=\'ajaxPost("switch","mechantTenace","ajaxMainSet");\'>
  <input id="mechantRiposte" type="button" value="'.$str['villainRetaliate'].'" onclick=\'ajaxPost("switch","mechantRiposte","ajaxMainSet");\'>
  <input id="mechantPercant" type="button" value="'.$str['villainPiercing'].'" onclick=\'ajaxPost("switch","mechantPercant","ajaxMainSet");\'>
  <input id="mechantDistance" type="button" value="'.$str['villainRanged'].'" onclick=\'ajaxPost("switch","mechantDistance","ajaxMainSet");\'>';
...
<div>
  <div id="pincipaleMenace">
    <input id='maCrise' type='button' value='<' onclick='document.getElementById("manigancePri").innerText-=1;ajaxPost("manigance",document.getElementById("manigancePri").innerText,"ajaxMainSet");'>
    <div id="manigancePri"></div>
    <input class='vieBtn' type='button' value='>' onclick='document.getElementById("manigancePri").innerText=parseInt(document.getElementById("manigancePri").innerText)+1;ajaxPost("manigance",document.getElementById("manigancePri","ajaxMainSet").innerText);'>
  </div>
  <div id="principaleAcc">
    <img src="img/Menace+.png"><div class="principaleAccButtons">
    <input class='vieBtn' type='button' value='<' onclick='document.getElementById("maniganceAcc").innerText-=1;ajaxPost("maniganceAcc",document.getElementById("maniganceAcc").innerText,"ajaxMainSet");'>
    <div id="maniganceAcc"></div>
    <input class='vieBtn' type='button' value='>' onclick='document.getElementById("maniganceAcc").innerText=parseInt(document.getElementById("maniganceAcc").innerText)+1;ajaxPost("maniganceAcc",document.getElementById("maniganceAcc").innerText,"ajaxMainSet");'> 
  </div></div>
  <div id="principaleMax">
    Max
    <input class='vieBtn' type='button' value='<' onclick='document.getElementById("manigancePriMax").innerText-=1;ajaxPost("maniganceMax",document.getElementById("manigancePriMax").innerText,"ajaxMainSet");'>
    <div id="manigancePriMax"></div>
    <input class='vieBtn' type='button' value='>' onclick='document.getElementById("manigancePriMax").innerText=parseInt(document.getElementById("manigancePriMax").innerText)+1;ajaxPost("maniganceMax",document.getElementById("manigancePriMax").innerText,"ajaxMainSet");'> 
  </div></div>
...
echo '<div id="changePhase"><div class="titlePopup">'.$str['changePhase'].'</div>'.$str['confirmPhase'].'<br/>'.$str['toChangePhase'].' <span id="changePhaseNext"></span>, <span id="changePhaseMechant"></span> '.$str['phaseChange1'].' <span id="changePhaseVie"></span> '.$str['pahseChange2'].' ?<br/><br/>
  <div class="boutonsPopup">
    <input type ="button" value="'.$str['confirm'].'" onclick=\'ajaxPost("phase",document.getElementById("changePhaseNext").innerText,"ajaxMainSet");\' class=\'bouton\'>
...
foreach ($xmlBoxes->box as $xmlBox) if ($xmlBox['own']==1) foreach ($xmlBox->mechant as $mechant) echo "<div class='changeMechant' onclick='ajaxPost(\"mechant\",".$mechant['id'].",\"ajaxMainSet\");'><img src='img/mechants/".$mechant['id'].".png'/>".$mechant['name'].'</div>';
...
foreach ($xmlBoxes->box as $xmlBox) if ($xmlBox['own']==1) foreach ($xmlBox->heros as $heros) echo "<div class='changeHeros' onclick='ajaxPost(\"joueurNum=\"+document.getElementById(\"herosAChanger\").value+\"&heros\",".$heros['id'].",\"ajaxMainSet\");'><img src='img/heros/".$heros['id'].".png'/>".$heros['name'].'</div>';
...
echo '<div id="NewPrincipale"><div class="titlePopup">'.$str['ChangeMScheme'].'</div><div class="centre">&#9888; '.$str['confirmMScheme'].' &#9888;</div><br/><select name="NewPrincipaleId" id="NewPrincipaleId"><option value="">--'.$str['mainScheme'].'--</option>';
  ...
  echo '</select><div class="boutonsPopup"><input type ="button" value="'.$str['confirm'].'" onclick=\'ajaxPost("NewPrincipale",document.getElementById("NewPrincipaleId").value,"ajaxMainSet");document.getElementById("NewPrincipale").style.display="none";\' class=\'bouton\'><input type="button" value="'.$str['cancel'].'" onclick=\'document.getElementById("NewPrincipale").style.display="none";\' class=\'bouton\'></div></div>';

echo '<div id="NewManigance">
  <div class="titlePopup">'.$str['AddSideScheme'].'</div><select name=\'deck\' id=\'deck\' onChange="ajaxCallCache (ajaxDeckSet,\'ajax/\'+encodeURIComponent(document.getElementById(\'partie\').value+\'.xml\'))"></select><select name="manigance" id="newManiganceId"></select><br/>
  <div class="boutonsPopup">
    <input type ="button" value="'.$str['confirm'].'" onclick=\'ajaxPost("newManigance",document.getElementById("newManiganceId").value,"ajaxMainSet");document.getElementById("newManiganceId").style.display="none";document.getElementById("NewManigance").style.display="none";document.getElementById("deck").value="0";\' class=\'bouton\' id=\'NewManiganceConfirm\' disabled>
    <input type="button" value="'.$str['cancel'].'" onclick=\'document.getElementById("NewManigance").style.display="none";\' class=\'bouton\'></div></div>';
...
echo '<div id="changeNameIndex"><div class="titlePopup">'.$str['changePlayerName'].'</div>'.$str['newPlayerName'].' <span id="changeNameOld"></span> :<br/><input type=\'text\' id=\'playerName\' minlength="4" maxlength="50"><input type=\'hidden\' id=\'changeNameId\'>
  <div class="centre"><input type ="button" value="'.$str['confirm'].'" onclick=\'ajaxPost("j="+document.getElementById("changeNameId").value+"&changeName",document.getElementById("playerName").value,"ajaxMainSet");document.getElementById("changeNameIndex").style.display="none";document.getElementById("playerName").value="";\'><input type="button" value="'.$str['cancel'].'" onclick=\'document.getElementById("changeNameIndex").style.display="none";\'></div></div>';
