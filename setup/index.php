...
<div id="changeMechant">
  ...
  <div class="centre">&#9888; Si vous changez de méchant, le nouveau sera réinitialisé à la phase 1 &#9888;<br/>Et le jeton premier joueur sera réattribué aléatoirement.</div><br/>
...
while ($mechant=mysqli_fetch_assoc($mechants)) {echo "<div class='changeMechant' onclick='ajaxPost(\"mechant\",".$mechant['mId'].");'><img src='img/mechants/".$mechant['mId'].".png'/>".$mechant['mNom'].'</div>';}
?>
  </div>
  <br/>
  <div class="boutonsPopup">
...
<div id="changeHeros">
  ...
  <div class="centre">&#9888; Si vous changez de personnage, la vie de ce joueur sera réinitialisée.</div><br/>
  <div id="herosSelect">
<?php
$heros=sql_get("SELECT * FROM `heros`,`boites` WHERE `hId`>0 AND `hBoite`=`bID` AND `bInclus`='1' ORDER BY `hNom` ASC");
while ($hero=mysqli_fetch_assoc($heros)) {echo "<div class='changeHeros' onclick='ajaxPost(\"joueurNum=\"+document.getElementById(\"herosAChanger\").value+\"&heros\",".$hero['hId'].");'><img src='img/heros/".$hero['hId'].".png'/>".$hero['hNom'].'</div>';}
?>
  </div>
  <br/>
  <div class="boutonsPopup">
...
<div id="NewPrincipale">
  ...
  <div class="centre">&#9888; Les valeurs actuelles de menace/maximum seront remplaçées. &#9888;</div><br/>
...
<div id="NewManigance">
  ...
  <select name='deck' id='deck' onchange='if (this.value=="0") {getElementById("newManiganceId").style.display="none";} else {ajaxCall(ajaxManigancesMenu,"p="+document.getElementById("partie").value+"&mGet="+(this.value));}'>
    ...
    <select name="manigance" id="newManiganceId"></select><br/>
  <div class="boutonsPopup">
...
<div id="changeNameIndex">
    ...
    <input type='text' id='playerName' minlength="4" maxlength="50">
